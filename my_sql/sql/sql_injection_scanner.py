        ...
        for phase in (GET, POST):
            ...
            for match in re.finditer(r"((\A|[?&])(?P<parameter>[^_]\w*)=)(?P<value>[^&#]+)", current):
                ...
                for b_a in BYPASS_ADDSLASHES:
                    ...
                    for (dbms, regex) in ((dbms, regex) for dbms in DBMS_ERRORS for regex in DBMS_ERRORS[dbms]):
                        if not vulnerable and re.search(regex, content[HTML], re.I) and not re.search(regex, original[HTML], re.I):
                            print(" (i) %s parameter '%s' appears to be error SQLi vulnerable (%s) -- bypass_addslashes" % (phase, match.group("parameter"), dbms))
                            retval = vulnerable = True
                            i = 1
                    if i == 1:
                        break
...
if __name__ == "__main__":
    ...
    print("By %s \n" % AUTHOR)
    USAGE = '[+]Please input : python sql_injection_scanner.py [your option] [your data] (e.g. python sql_injection_scanner.py -u "http://43.247.91.228:84/Less-1/?id=1")'
    ...
    if options.url:
        ...
        result = scan(options.url if options.url.startswith("http") else "http://%s" % options.url, options.data)
