...
class UploadForm:
    def __init__(self, notRegex, trueRegex, session, size, postData,
        ...
        self.httpRequests = 0
        self.codeExecUrlPattern = None  # Pattern for code exec detection using true regex findings #代码执行检测的模式
        self.logLock = Lock()
    ...
    def setup(self, initUrl):
        ...
        if len(detectedForms[0][1]) > 1:
            ...
            exit()
        #self.inputName = detectedForms[0][1]["name"]
        #print(detectedForms[0][1][0])
        #print(detectedForms[0][1][0]['name'])
        ...
        if not self.uploadsFolder and not self.trueRegex: 
            ...
            print("No uploads path provided, code detection can still be done "
                  ...
                  "(Except for templates with a custom codeExecURL)")
            cont = input("Do you want to use the True Regex for code execution detection ? [Y/n] ")#直接回车也是y
            if cont.lower().startswith("y") or cont == "":
                prefixPattern = input("Prefix capturing group of the true regex with: ")#前缀
                suffixPattern = input("Suffix capturing group of the true regex with: ")#后缀
                self.codeExecUrlPattern = "".join((prefixPattern, "$captGroup$", suffixPattern))#元组加进去，依次接在后面；中间有一个捕获组标识
            else:
    ...
    def uploadFile(self, suffix, mime, payload,
        ...
        with tempfile.NamedTemporaryFile(suffix=suffix) as fd:
            ...
            if dynamicPayload:
                payload = payload.replace(b"$filename$", bytearray(filename_wo_ext, 'ascii'))# byte字节符
            fd.write(payload)
            fd.flush()
            fd.seek(0)
            if self.shouldLog:
            ...
            #print(fileUploadResponse.text)
            self.httpRequests += 1#请求数增加
            if self.shouldLog:
                if self.logger.verbosity > 1:
                    printSimpleResponseObject(fileUploadResponse)
                if self.logger.verbosity > 2:
                    print(f"\033[36m{fileUploadResponse}\033[0m")
    ...
    def detectValidExtension(self, future):
        if not self.stopThreads:
            ...
            if r:
                ...
                if self.shouldLog:
                    self.logger.info("\033[1m\033[34mExtension %s is valid.\033[0m", ext)
                    '''
                    if r != True:
                        self.logger.info("\033[1;34mTrue regex matched the following information: %s\033[0m", r)
                    '''
            return r
    ...
    def detectValidExtensions(self, extensions, maxN, extList=None):
        ...
        ##########################线程池############### 最多并行执行的worker数量
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.threads) as executor:#启动并行任务；池的概念主要目的是为了重用：让线程或进程在生命周期内可以多次使用
            future_list = []
            try:
    ...
    def detectCodeExec(self, url, regex):
        """Detect if a code execution is gained, given an url to request and a
...
##################测试代码在对方上面执行
    def submitTestCase(self, suffix, mime,
        ...
        if self.shouldLog:
            self.logger.info("\033[1;34mUpload of '%s' with mime type %s successful\033[m", fu[1], mime)# 某个mime类型的文件xxx已经上传成功

        '''
        if uploadRes is True and self.shouldLog:
            self.logger.info("\033[1;34m\tTrue regex matched the following information: %s\033[m", uploadRes)
        '''
            ...
            if self.uploadsFolder or codeExecURL:
                if codeExecURL:
                    ...
                    url = codeExecURL.replace("$uploadFormDir$", os.path.dirname(self.uploadUrl)) \
                                     .replace("$filename$", filename_wo_ext)#把这个特殊位置给替代了；# 
                else:
