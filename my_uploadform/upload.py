...
templatesFolder = r"D:\myCTF\myscanner\my_uploadform\store\payloads"
with open(r"D:\myCTF\myscanner\my_uploadform\store\templates.json", "r", encoding='utf-8') as fd:
...
requiredNamedArgs.add_argument("--wrong-regex",  metavar="regex",  help="Regex matching an upload failure",  type=valid_regex, dest="notRegex")
...
exclusiveArgs.add_argument("-n", metavar="n", nargs=1, default=["200"], dest="n", help="Number of common extensions to use. Example: -n 100",  type=valid_nArg)
###############################################################
...
parser.add_argument("-T", "--threads", metavar="Threads", nargs=1, dest="nbThreads", help="Number of parallel tasks (threads).", type=int, default=[10])
...
if args.randomUserAgent:
    with open(r"D:\myCTF\myscanner\my_uploadform\store\user-agents.txt","r") as fd:
        nb = 0
        for l in fd:
            nb += 1
        fd.seek(0)
...
{Laevateinn: a ezScanner By hans99_ws0407_xxrw :) Enjoy}
""")
...
#mimeFile = r"D:\myCTF\myscanner\my_uploadform\store\mimeTypes.advance" 
mimeFile = r"D:\myCTF\myscanner\my_uploadform\store\mimeTypes.basic"#.basic是基本level advance是高级level
...
entryPoints = []
up.stopThreads = True
with open(r"D:\myCTF\myscanner\my_uploadform\store\techniques.json", "r") as rawTechniques:
...
####################################################
###第二层循环：即找到之前定好的可靠扩展
###第三层循环：如果有nasty扩展的话还要有匹配的technique
###第四层循环：对于每一个technique，其对应的每一个nasty variant扩展都要考虑在内
###核心代码逻辑================================================================================>
        ...
        for future in concurrent.futures.as_completed(future_list):
            ...
            if not stopThreads:
                if res["codeExec"]:
                    ...
                    logging.info("\033[1m\033[44mCode execution obtained ('%s','%s','%s','%s')\033[0m",
                                    foundEntryPoint["suffix"],#后缀 .extension
                                    foundEntryPoint["mime"],#mime类型
                                    foundEntryPoint["templateName"],#模板名
                                    res["url"])
